            ...
            try:
                ...
                if (A.det() != 0):
                    ...
                    if (A.rank() == A_.rank() and A.rank() == int(sopt.get())):
                        ...
                        print(X)
                        for i in range(int(sopt.get())):
                            Label(win,background = '#E3F7F2', font = ('Times New Roman', 12), fg = 'White', text = f'x{i} = ', width = 4).place( x = 280 + 83*i, y = 650)#grid(row = i, column = 4)
                            Label(win,background = '#E3F7F2', font = ('Times New Roman', 12), fg = 'White', text = f'{float(round((X[i]), 3))}' + ', ', width = 4).place( x = 320 + 83*i, y = 650)#grid(row = i, column = 4)
                    if (A.rank() == A_.rank() and A.rank() < int(sopt.get())):
...
def clear():
    ...
    Label(win,background = '#E3F7F2', width = 100).place( x = 280, y = 650)
    A = []
    entries_A = []
    B = []
    entries_B = []
